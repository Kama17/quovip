...
const AdminDashboard: React.FC = () => {
  ...
  const memberChatIds = new Set(selectedUser?.chats?.map((c) => c.chat_id));
  ...
  const fetchBotChats = async () => {
    ...
    try {
      const { data, error } = await supabase.from('bot_chats').select(`
        *,
        chat_members!left (
          user_id
        )
  ...
  const removeUserFromChat = async (chatId: string, telegramUserId: string, user_id: string) => {
    ...
    if (!data.ok) {
      ...
      const { error } = await supabase
        .from('chat_members')
        .update({ is_member_active: 'inactive' })
        .eq('chat_id', chatId)
        .eq('user_id', user_id);
      if (error) {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    return (
      ...
      (user.jifu_id?.toLowerCase().includes(text) ?? false) ||
      (user.telegram_name?.toLowerCase().includes(text) ?? false)
    );
  });
  ...
  const handleAddUser = async (values: any) => {
    try {
      if (selectedUser) {
        ...
        const { error } = await supabase.from('users').update(values).eq('id', selectedUser.id);
  ...
  const handleSend = async (messageToSend: string, targets: BackendPayload[], image?: File) => {
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <FloatButton.Group icon={<MoreOutlined />} closeIcon={<CloseOutlined />} trigger="click">
        <Popconfirm
          ...
          cancelText="No"
        >
          <FloatButton icon={<LogoutOutlined style={{ color: 'red' }} />} />
        </Popconfirm>
        <FloatButton
          icon={<TeamOutlined />}
          //description="Users"
          ...
          {selectedMenu === 'users' && (
            <>
              <Divider dashed>
                {selectedChatId === 'all'
                  ? 'All members'
                  : botChats.find((chat) => chat.chat_id === selectedChatId)?.chat_name}
              </Divider>
              {/* Users list */}
              <List
            ...
            <>
              ...
              <List
                ...
                renderItem={(user) => (
                  <List.Item
                    actions={[
                      <Popconfirm title="Cancel invite?" onConfirm={() => handleDeleteUser(user)}>
      ...
      >
        {selectedUser && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <Text>
              <b>Jifu ID:</b> {selectedUser.jifu_id}
            </Text>
            <Text>
      ...
      <Drawer
        ...
        width={360}
        footer={
          <List.Item style={{ cursor: 'pointer' }} onClick={() => setSelectedChatId('all')}>
            <List.Item.Meta title="All members" />
          </List.Item>
        }
      >
          ...
          {!selectedUser && ( // Maby not to invate then adding new user. Maby add after user is verified
            ...
            >
              {loadingBotChats ? (
                ...
                <Select mode="multiple" placeholder="Select one or more chats" allowClear>
                  {botChats.map((chat) => (
      ...
      >
        Are you sure you want to delete <b>{selectedUser?.user_name || selectedUser?.first_name}</b>
        ?
      </Modal>
    </Layout>
  );
